...
def get_config() -> ExperimentConfig:
    ...
    config.net = LazyConfig(NSADeepSeekSpatialViT)(
        ...
        num_routed=4,
        top_k_moe=2,
        kernel_size_nsa=64,      # Compression kernel size (must be in {16, 32, 64, 128})
        kernel_stride_nsa=32,    # Stride for compression
        block_size_nsa=64,       # Block size for sparse attention
        window_size_nsa=128,     # Sliding window size
        top_k_nsa=4,             # Top-k blocks for sparse attention
    )
